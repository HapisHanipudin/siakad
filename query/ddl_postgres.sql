-- =============================================================================
-- PostgreSQL DDL & DML Seeding Script (academic database)
-- =============================================================================

-- Drop tables if they already exist to avoid errors (with CASCADE for automatic FK resolution)
...
CREATE TABLE program_studi (
    ...
    jenjang VARCHAR(5) NOT NULL CHECK (jenjang IN ('D3', 'D4', 'S1', 'S2', 'S3')),
...
CREATE TABLE dosen (
    ...
    gelar VARCHAR(5) NOT NULL CHECK (gelar IN ('D3', 'D4', 'S1', 'S2', 'S3')),
    FOREIGN KEY (id_fakultas) REFERENCES fakultas(id_fakultas) ON DELETE RESTRICT ON UPDATE CASCADE
);
...
CREATE TABLE ruangan (
    ...
    tipe_ruangan VARCHAR(20) NOT NULL CHECK (tipe_ruangan IN ('kelas_biasa', 'laboratorium')),
...
CREATE TABLE prasyarat_mata_kuliah (
    ...
    nilai_min VARCHAR(2) CHECK (nilai_min IN ('A', 'B', 'C', 'D', 'E', 'F')),
...
CREATE TABLE kurikulum_mata_kuliah (
    ...
    tipe_mata_kuliah VARCHAR(20) NOT NULL CHECK (tipe_mata_kuliah IN ('wajib', 'peminatan')),
...
CREATE TABLE mahasiswa (
    ...
    status_mahasiswa VARCHAR(20) DEFAULT 'aktif' CHECK (status_mahasiswa IN ('aktif', 'cuti', 'lulus', 'drop_out')),
    angkatan INT NOT NULL,
    FOREIGN KEY (id_program_studi) REFERENCES program_studi(id_program_studi) ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE kelas (
    ...
    kuota INT NOT NULL,
    semester_aktif VARCHAR(10) NOT NULL CHECK (semester_aktif IN ('ganjil', 'genap')),
...
CREATE TABLE pengumuman (
    ...
    target VARCHAR(20) DEFAULT 'global' CHECK (target IN ('global', 'prodi', 'personal')),
...
CREATE TABLE krs (
    ...
    id_tahun_ajaran INT NOT NULL,
    semester_aktif VARCHAR(10) NOT NULL CHECK (semester_aktif IN ('ganjil', 'genap')),
    status_krs VARCHAR(20) NOT NULL CHECK (status_krs IN ('draft', 'menunggu', 'ditolak', 'sah')),
    catatan TEXT NULL,
...
CREATE TABLE detail_krs (
    ...
    nilai_akhir_huruf VARCHAR(2) NULL CHECK (nilai_akhir_huruf IN ('A', 'B', 'C', 'D', 'E', 'F')),
...
CREATE TABLE presensi (
    ...
    status_presensi VARCHAR(10) NOT NULL CHECK (status_presensi IN ('hadir', 'izin', 'sakit', 'alfa')),
...
CREATE TABLE tagihan (
    ...
    id_tahun_ajaran INT NOT NULL,
    semester_aktif VARCHAR(10) NOT NULL CHECK (semester_aktif IN ('ganjil', 'genap')),
    tipe_tagihan VARCHAR(10) NOT NULL CHECK (tipe_tagihan IN ('ukt', 'spi', 'denda')),
    nominal DECIMAL(10,2) NOT NULL,
    status_tagihan VARCHAR(20) NOT NULL CHECK (status_tagihan IN ('belum', 'diverifikasi', 'lunas')),
...
CREATE TABLE pembayaran (
    ...
    status_pembayaran VARCHAR(20) NOT NULL CHECK (status_pembayaran IN ('belum', 'diverifikasi', 'lunas')),
